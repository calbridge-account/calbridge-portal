...
(async () => {
  ...
  const userId    = uuidv4();
  const defaultPassword = process.env.AGENCY_DEFAULT_PASSWORD || require('crypto').randomBytes(16).toString('hex');
  ...
  console.log('   Password: (set via AGENCY_DEFAULT_PASSWORD env var or random if not set)');
