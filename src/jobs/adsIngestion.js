...
REPORT_TYPES.push(
  ...
  },
  {
    ...
    // Fixed 2026-04-27: SB reports use purchases/sales (not windowed purchases14d/sales14d — SP-only)
    groupBy: ['campaign_bid_boost_segment'],
    columns: ['campaignId', 'campaignName', 'segmentName', 'segmentClassCode', 'impressions', 'clicks', 'cost', 'purchases', 'sales', 'date'],
...
async function writeSpCampaignReport(clientId, profileId, reportDate, rows) {
  ...
  const rawRows = rows.map(r => ({
    ...
    portfolio_id:                  r.portfolioId ? String(r.portfolioId) : null,
    top_of_search_impression_share: r.topOfSearchImpressionShare || null,
    units_sold_1d:                 r.unitsSoldClicks1d || null,
    units_sold_7d:                 r.unitsSoldClicks7d || null,
    units_sold_14d:                r.unitsSoldClicks14d || null,
    units_sold_30d:                r.unitsSoldClicks30d || null,
    marketplace_id:                (r.campaignBudgetCurrencyCode === 'CAD') ? 'A2EUQ1WTGCTBG2' : 'ATVPDKIKX0DER',
  }));
  return writeRawAdCampaign(rawRows);
}
...
async function writeSbCampaignReport(clientId, profileId, reportDate, rows) {
  ...
  const rawRows = rows.map(r => ({
    ...
    video_5s_views:                r.video5SecondViews || null,
    video_25pct_views:             r.videoFirstQuartileViews || null,
    video_50pct_views:             r.videoMidpointViews || null,
    video_75pct_views:             r.videoThirdQuartileViews || null,
    video_100pct_views:            r.videoCompleteViews || null,
    video_unmutes:                 r.videoUnmutes || null,
    viewability_rate:              r.viewabilityRate || null,
    top_of_search_impression_share: r.topOfSearchImpressionShare || null,
    branded_searches:              r.brandedSearches || null,
    brand_store_page_views:        r.brandStorePageView || null,
  }));
  return writeRawAdCampaign(rawRows);
}
...
async function writeSdCampaignReport(clientId, profileId, reportDate, rows) {
  ...
  const rawRows = rows.map(r => ({
    ...
    add_to_cart:            r.addToCart || null,
    viewability_rate:       r.viewabilityRate || null,
    purchases_promoted:     r.purchasesPromotedClicks || null,
    sales_promoted:         r.salesPromotedClicks || null,
    video_25pct_views:      r.videoFirstQuartileViews || null,
    video_50pct_views:      r.videoMidpointViews || null,
    video_75pct_views:      r.videoThirdQuartileViews || null,
    video_100pct_views:     r.videoCompleteViews || null,
...
async function writeRawAdCampaign(rows) {
  ...
  return batchMerge({
    ...
    dataColumns: [
      'platform', 'marketplace', 'marketplace_id', 'campaign_name', 'status',
      'daily_budget', 'profile_id',
      'campaign_budget_currency_code', 'campaign_budget_type',
      'campaign_bidding_strategy', 'portfolio_id',
      'impressions', 'clicks', 'cost',
      ...
      'top_of_search_impression_share',
      'new_to_brand_purchases', 'new_to_brand_purchases_clicks',
      'new_to_brand_sales', 'new_to_brand_units_sold',
      'detail_page_views', 'add_to_cart',
      'video_5s_views', 'video_25pct_views', 'video_50pct_views',
      'video_75pct_views', 'video_100pct_views', 'video_unmutes',
      'viewability_rate', 'branded_searches', 'brand_store_page_views',
      'purchases_promoted', 'sales_promoted',
      ...
      'viewable_impressions', 'ingested_at', 'data_maturity', 'last_refreshed_at',
    ],
    rows,
  });
}
